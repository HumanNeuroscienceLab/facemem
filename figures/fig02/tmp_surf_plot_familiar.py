...
# This script will plot the famliliar (day one remember or not remember results)
...
for runtype in runtypes:
    ...
    indir = "%s/analysis/groups/%s/task/%s_familiarity_bio_rem.mema/easythresh" % (basedir, runtype, lruntype)
    thr_file = "%s/thresh_zstat_yes_vs_no.nii.gz" % indir

    indir2 = "%s/analysis/groups/%s/task/%s_familiarity_bio_rem.mema" % (basedir, runtype, lruntype)
    raw_file = "%s/zstats_yes_gt_no.nii.gz" % indir2

    outdir = "%s/figures/scratch" % basedir
