...
for runtype in runtypes:
    ...
    indir = "%s/analysis/groups/%s/connectivity/tconn_%s" % (basedir, runtype, region)
    raw_file = "%s/zstats_parcels_397_bio_gt_phys.nii.gz" % indir
    thr_file = "%s/easythresh_liberal/thresh_zstats_parcels_397_bio_vs_phys.nii.gz" % indir
    ...
    for hemi in hemis:
        ...
        zstat = project_thresholded_data(raw_file, thr_file, hemi, subject_id='mni152', 
                                         target_subject='fsaverage', smooth_fwhm=2, 
                                         verbose=2)
