...
# Converts AFNI style hex to the rgb format desired by pysurfer
afni2rgb <- function(ifn, ofn=NULL) {
  cols  <- as.character(read.table(ifn, header=T, comment="$")[,])
  rcols <- col2rgb(cols)
  rcols <- t(rcols)
  if (is.null(ofn)) ofn <- sub(".pal", ".txt", ifn)
  write.table(rcols, file=ofn, row.names=F, col.names=F, quote=F)
}
