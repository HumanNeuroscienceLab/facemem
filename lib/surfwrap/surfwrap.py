...
class SurfWrap(object):
    ...
    def set_underlay(self, subject_id="cvs_avg35_inMNI152", surf="inflated", 
                     subjects_dir=None):
    ...
    def set_overlay(self, name, volume_file, cbar, min='auto', max='auto', sign="auto"):
        ...
        if min == 'auto' or max == 'auto' or sign == 'auto':
            ...
            data = img.get_data()
            dmin = data.min()
            data_max = np.abs(data[data.nonzero()]).max()
            if data_max == 0:
                data_min = data_max
            elif dmin < 0:
                data_min = np.abs(data[data.nonzero()]).min()
            else:
                data_min = data[data.nonzero()].min()
            if max == 'auto': max = data_max
            if min == 'auto': min = data_min
            if sign == 'auto':
                if dmin < 0 and data_max > 0:
                    sign = "abs"
                elif dmin > 0:
                    sign = "pos"
                else:
